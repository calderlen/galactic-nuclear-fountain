...
const SPARC_FILE =
    joinpath(INPUT, "sparc", "SPARC_Lelli2016c.mrt")

const SPARC_RC_FILE =
    joinpath(INPUT, "sparc", "MassModels_Lelli2016c.mrt")

const THINGS_FILE =
    joinpath(INPUT, "things", "Galaxies.dat")

const LEROY_FILE =
    joinpath(INPUT, "leroy2008", "GlobalGalaxyQuantities.dat")
...
function read_sparc(path)
    ...
    for line in eachline(path)
        ...
        any(isnothing, (T, D, inc, Rdisk, MHI, RHI, Vflat, Q)) && continue
...
println("SPARC galaxies:      $(nrow(sparc))")
println("THINGS galaxies:     $(nrow(things))")
println("Leroy08 galaxies:    $(nrow(leroy))")
println("Triple overlap:      $(nrow(triple))")
println("SPARC RC points:     $(nrow(rotation_curves))")
println("Leroy radial points: $(nrow(radial_profiles))")